...
// ImgOptionModifier 是用于修改 ImgOption 的函数类型
...
func (o *ImgOption) CreateBasePhoto() *gg.Context {
	...
	o.Image.DrawStringAnchored("15:00—16:40", 45, 475, 0, 0)
	o.Image.DrawStringAnchored("16:50—18:30", 45, 615, 0, 0)
	o.Image.DrawStringAnchored("19:30—21:10", 45, 850, 0, 0)
...
func (o *ImgOption) CheckData(data map[string]any) (dataClass map[string]any) {
	...
	photoPath := utils.ImgPath(o.TemplateImgPath)
...
func (o *ImgOption) Create(cnameData map[string]any) (string, *gg.Context) {
	...
	for weekDay, dayName := range cnameData["课程信息"].(map[string]interface{})["星期数据"].([]interface{}) {
		...
		for timeSlot, courseInfo := range dayInfo {
			...
			context.DrawStringAnchored(lessonName, weekPlaceData[weekDay]+5, hPlaceData[timeSlot]+30, 0, 0)
			context.DrawStringAnchored(teacher, weekPlaceData[weekDay]+10, hPlaceData[timeSlot]+65, 0, 0)
			context.DrawStringAnchored(weekInfo, weekPlaceData[weekDay]+10, hPlaceData[timeSlot]+95, 0, 0)
			context.DrawStringAnchored(place, weekPlaceData[weekDay]+10, hPlaceData[timeSlot]+125, 0, 0)
		}
	}
